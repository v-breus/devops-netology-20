...
* cost - оценочная продолжительность операции в секундах
* rows - количество просканированных строк
* width - размер строки в байтах

Команда сканирует 298 строк базы длиной в 244 байта за 0-13 секунд (показано стрелкой) и применяет к итогу фильтр "orders is not null". Не особенно эффективно, лучше сначала сформировать индекс:

```
CREATE INDEX ON clients(orders);
```
Тогда обработка потребует сканирования уже только 5 строк за 1.05 секунд (показано стрелкой)
